...
function 法术界面:初始化()
    self:置精灵(self:取拉伸精灵_宽高('gires/main/border.bmp', 420, 350))
end

local x, y = 5, 5
local 小叹号 = __res:getspr('gires4/ty/xth.tcp')
for i, v in ipairs { '技能1', '技能2', '技能3', '法门', '天赋', '其它' } do
    local 区域 = 法术界面:创建控件('区域' .. i, x, y, 140, 180)
    do
        local 技能 = 区域:创建文本('文本', 0, 0, 20, 150)
        技能:置文字(__res.F16B)
        技能:置文本('#cCBB55B' .. v)
        local 列表 = 区域:创建列表('列表', 20, 0, 120, 150)
        列表.行高度 = 28
        列表.行间距 = 3
        列表.选中精灵 = nil
        列表.焦点精灵 = nil
...
function 窗口层:打开人物法术界面(nid)
    法术界面:置可见(true)
    法术界面.区域1:置宽高(140,180)
    法术界面.区域1.列表:置宽高(120,150)
    法术界面.是否召唤 = nil
    for i = 1, 6 do
        local t = 法术界面['区域' .. i]
        t.列表:清空()
    ...
    法术界面:置精灵(self:取拉伸精灵_宽高('gires/main/border.bmp', 420, 350), true)
    法术界面:置坐标((引擎.宽度 - 法术界面.宽度) // 2, (引擎.高度 - 法术界面.高度) // 2)

    local list, MP, HP = __rpc:角色_战斗技能列表(nid)
    ...
    for i, v in ipairs(nlist) do
        local 区域 = 法术界面['区域' .. v.区域]
        if 区域 then
            区域.文本:置文本('#cCBB55B' .. v.缩写)
            区域.列表:添加技能(v)
        end
    end
...
function 窗口层:打开召唤法术界面()
    法术界面:置可见(true)
    法术界面.是否召唤 = 战场层.操作目标.nid
    for i = 3, 6 do
        法术界面['区域' .. i]:置可见(false)
    end
    法术界面.区域1:置宽高(140,280)
    法术界面.区域1.列表:置宽高(120,270)
    法术界面.区域1.文本:置文本('#cCBB55B召唤兽')
    法术界面.区域2.文本:置文本('#cCBB55B其它')
    法术界面:置精灵(self:取拉伸精灵_宽高('gires/main/border.bmp', 290, 290), true)
    法术界面:置坐标((引擎.宽度 - 法术界面.宽度) // 2, (引擎.高度 - 法术界面.高度) // 2)

    local list, MP, HP = __rpc:召唤_战斗技能列表(战场层.操作目标.nid)
