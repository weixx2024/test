...
local _说明 = {
    ...
    [51] =
    "红拂女#r【种族】人【性别】女#r【武器】鞭或剑#r此女为风尘三侠之一，以女流之身而修武入道，亦侠亦仙，属传说中的人物。其性情刁钻、武艺高强，见不得一点不平事，听不得一些是非语。手中长鞭有如龙蛇，矫灵异常，实为无知宵小的克星。",
    ...
    [52] =
    "混天魔#r【种族】魔【性别】男#r【武器】刀或斧#r天庭诞生之前就存在的混世魔王，因屡屡扰乱天地和谐曾被元始天尊封印了九万九千九百年。形体极为高大修长，身着亮银魔甲，手执六鬼锁元刀，加上冷竣的眼神，给人一种不寒而栗的感觉。",
    ...
    [55] =
    "霓裳仙#r【种族】仙【性别】女#r【武器】飘带或灯笼#r玉皇大帝之女，娇柔之状，我见尤怜。因自幼挚爱纺织，遂采白云为布、积月光为丝、以彩虹染色织就霓裳。难以想象一位那么娟秀的女子，一双如此柔弱的纤手竟能释放出那么强大的法术。",
    ...
    [64] =
    "幽梦影#r【种族】鬼【性别】女#r【武器】环或飘带#r生于深宅大院，自幼受到良好的教育，才名四起，被人比作再世谢道韫。正值豆蔻年华，却被父母指婚嫁给纨绔子弟。心中另有所爱的她不愿成为政治婚姻的牺牲品，再几经反抗却无计可施之后，自尽抗婚。",
...
local 转种族窗口 = 窗口层:创建我的窗口('转种族窗口', 0, 0, 495, 419)
...
function 转种族窗口:可见事件(v)
    if v == false and co then
        coroutine.resume(co)
    end
end

local 标签控件 = 转种族窗口:创建标签('标签控件', 0, 0, 495, 419)
...
do
    ...
    do
        ...
        function 男人区域:前显示(x, y)
            if self.选中 then
                转种族窗口.选中框:显示(x + _坐标组[self.选中].x, y + _坐标组[self.选中].y)
            end
        end
    end
...
do
    local 文本 = 转种族窗口:创建文本("武器", 210, 243, 14, 70)
    文本:置文本("武器预览")
    文本 = 转种族窗口:创建文本("介绍文本", 215, 78, 232, 120)
    文本:置文本("逍遥生#r【种族】人【性别】男#r【武器】扇或剑#r一袭白衫一把铁扇或一柄长剑,凭此三物而走遍江湖。与一般武夫不同的是,他们在快意恩仇之余,也喜欢舞文弄墨。")
end
local 转种族按钮 = 转种族窗口:创建中按钮("转种族按钮", 298, 357, "转种族")
function 转种族按钮:左键弹起()
    if co then
        coroutine.resume(co, _外形, _种族, _性别)
        self.父控件:置可见(false)
    end
end

function 转种族窗口:更新(dt)
    if self.动画1 then
        self.动画1:更新(dt)
    end
    if self.动画2 then
...
function 转种族窗口:置模型(n)
    ...
    转种族窗口.动画1 = require('对象/基类/动作') { 外形 = n, 模型 = 'attack' }
    转种族窗口.动画1:置循环(true)
    转种族窗口.动画2 = require('对象/基类/动作') { 外形 = n + a, 模型 = 'attack' }
    转种族窗口.动画2:置循环(true)
    self.介绍文本:置文本(_说明[n])
    _外形 = n
end
...
function 窗口层:打开转种族窗口()
    转种族窗口:置可见(not 转种族窗口.是否可见)
    if not 转种族窗口.是否可见 then
        return
    end
    local n = __rpc:角色_取转生()
    置角色数量(n)
    人族按钮:置选中(true)
end
