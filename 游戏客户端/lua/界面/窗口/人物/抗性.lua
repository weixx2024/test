...
local 人物抗性 = 窗口层:创建我的窗口('人物抗性', 0, 0, 289, 800)
function 人物抗性:初始化()
    self:置坐标(引擎.宽度 - 362 - 289, (引擎.高度 - 455) // 2)
end

for k, v in ipairs(抗性类型) do
    local y = k == 1 and 24 or 0
    local 抗性列表项 = 人物抗性:创建多列列表(v, 5, y, 289, 666)
    ...
    function 抗性列表项:初始化()
        ...
        self:添加列(0, 2, 92, 14)
        self:添加列(92, 2, 58, 14)
        self:添加列(147, 2, 92, 14)
        self:添加列(239, 2, 47, 14)
    end
...
for k, v in ipairs(抗性类型) do
    local 抗性长按钮 = 人物抗性:创建抗性长按钮('抗性长按钮' .. k, 0, (k - 1) * 24, v)
    table.insert(table_抗性长按钮, 抗性长按钮)
end

function 窗口层:重排人物抗性按钮()
    local 总高度 = 0
...
function 人物抗性:添加抗性(r)
    ...
    人物抗性:置精灵(
        生成精灵(
            ...
            function()
                人物抗性:取拉伸图像_宽高('gires4/经典红木/元件按钮/透明框_2.tcp', 289,
...
function 窗口层:重新打开人物抗性()
    人物抗性:置坐标(引擎.宽度 - 362 - 289, (引擎.高度 - 455) // 2)
    高度列表 = { 0, 0, 0, 0, 0 }
