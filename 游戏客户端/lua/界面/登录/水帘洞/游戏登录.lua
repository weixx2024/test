...
function 游戏登录:初始化()
    ...
    self:置精灵(__res:getspr('gires2/login/passwd.tcp'))
end
...
local 记住账号 = 游戏登录:创建多选按钮('记住账号', 530, 180)
do
    function 记住账号:初始化()
        local tcp = __res:get('ui2/多选框.tcp')
...
local 记住密码 = 游戏登录:创建多选按钮('记住密码', 530, 216)
do
    function 记住密码:初始化()
        local tcp = __res:get('ui2/多选框.tcp')
        self:置正常精灵(tcp:取精灵(1))
...
local 账号输入 = 游戏登录:创建文本输入('账号输入', 272, 186, 240, 16)
do
    function 账号输入:初始化()
...
local 密码输入 = 游戏登录:创建文本输入('密码输入', 272, 223, 240, 16)
do
    function 密码输入:初始化()
        self:置颜色(255, 255, 255, 255)
...
local 登录按钮 = 游戏登录:创建按钮('登录按钮', 252, 275)
do
    function 登录按钮:初始化()
...
local 取消按钮 = 游戏登录:创建按钮('取消按钮', 350, 275)
do
    function 取消按钮:初始化()
...
local 离开按钮 = 游戏登录:创建按钮('离开按钮', 448, 275)
do
    function 离开按钮:初始化()
        self:设置按钮精灵('gires2/login/exit.tca')
    end
...
do
    ...
    local 下拉按钮 = 游戏登录:创建按钮('下拉按钮', 504, 184, 18, 18)
    function 下拉按钮:初始化()
...
游戏登录:创建文本('提示文本', 100, 40, 400, 200):置文字(__res.F18):置文本(
    '#G温馨提醒:\n#Y为了防止您在输入账号、密码时被不明程序监听，请启用“密码保护”功能选项。如果玩家在启用该功能后无法正常输入账号和密码,证明有其他程序正在监视玩家的键盘输入，请完假确认该程序的合法性。')
