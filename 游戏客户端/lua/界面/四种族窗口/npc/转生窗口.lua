local 转生窗口 = 窗口层:创建我的窗口('转生窗口', 0, 0, 448, 367)
local _种族 = { "人", "魔", "仙", "鬼" }
...
local 修正 = {
    ...
    [4002] = {
        ...
        { 抗鬼火 = 27.68, 抗遗忘 = 20.5, 反震率 = 1025, 反震程度 = 20.5 }
    }
}
function 转生窗口:初始化()
...
do
    for k, v in pairs {
        ...
        local 文本 = 转生窗口:创建文本(v.name .. "文本", v.x, v.y, 90, 14):置文本("未知")
    end


    local 文本 = 转生窗口:创建文本("抗性修正文本", 17, 213, 200, 38)
    文本 = 转生窗口:创建文本("属性修正文本", 17, 285, 200, 38)
    文本 = 转生窗口:创建文本("来世抗性文本", 233, 213, 200, 38)
    文本 = 转生窗口:创建文本("来世属性文本", 233, 285, 200, 38)

    local 确定按钮 = 转生窗口:创建小按钮("确定按钮", 147, 332, "确定")
    ...
    local 取消按钮 = 转生窗口:创建小按钮("取消按钮", 253, 332, "取消")
    local 重选按钮 = 转生窗口:创建小按钮('重选按钮', 14, 334, '重选角色')
    function 重选按钮:初始化()
...
function 转生窗口:刷新转生来世(种族, 性别, 外形)
    _来世种族, _来世性别, _来世外形 = 种族, 性别, 外形
    local _来世等级 = { 15, 15, 60 }
    for k, v in pairs { "根骨2", "灵性2", "力量2", "敏捷2",

    } do
        转生窗口[v .. "文本"]:置文本(_来世等级[_转生 + 1])
    end
    转生窗口.种族2文本:置文本(_种族[_来世种族])
    转生窗口:置头像(nil, _来世外形)
    local 临时记录 = {}
    ...
    self.来世抗性文本:置文本(转生窗口:取修正(kx))
    self.来世属性文本:置文本(转生窗口:取修正(sx))
end

function 窗口层:打开转生窗口()
    转生窗口:置可见(not 转生窗口.是否可见)
    if not 转生窗口.是否可见 then
        return
    end
    ...
    --置今生
    for k, v in pairs(t) do
        if type(v) ~= "table" and 转生窗口[k .. "文本"] then
    ...
    转生窗口.抗性修正文本:置文本(转生窗口:取修正(kx))
    转生窗口.属性修正文本:置文本(转生窗口:取修正(sx))


    --置来世
    转生窗口:刷新转生来世(t.种族, t.性别, t.头像)
end

function 窗口层:刷新转生来世(种族, 性别, 外形)
    if not 转生窗口.是否可见 then
        return
    end
    转生窗口:刷新转生来世(种族, 性别, 外形)
end
