...
local 飞升重选窗口 = 窗口层:创建我的窗口('飞升重选窗口', 0, 0, 495, 419)
...
function 飞升重选窗口:可见事件(v)
    -- if v == false and co then
    --     coroutine.resume(co)
    -- end
end

local 标签控件 = 飞升重选窗口:创建标签('标签控件', 0, 0, 495, 419)
...
do
    ...
    do
        ...
        function 男人区域:前显示(x, y)
            if self.选中 then
                飞升重选窗口.选中框:显示(x + _坐标组[self.选中].x, y + _坐标组[self.选中].y)
            end
        end
...
do
    local 文本 = 飞升重选窗口:创建文本("武器", 210, 243, 14, 70)
    文本:置文本("武器预览")
    文本 = 飞升重选窗口:创建文本("介绍文本", 215, 78, 232, 120)
    文本:置文本("逍遥生#r【种族】人【性别】男#r【武器】扇或剑#r一袭白衫一把铁扇或一柄长剑,凭此三物而走遍江湖。与一般武夫不同的是,他们在快意恩仇之余,也喜欢舞文弄墨。")
end
local 转种族按钮 = 飞升重选窗口:创建中按钮("转种族按钮", 298, 357, "确  定")
function 转种族按钮:左键弹起()
    窗口层:刷新飞生来世(_种族, _性别, _外形)
    self.父控件:置可见(false)
end

function 飞升重选窗口:更新(dt)
    if self.动画1 then
        self.动画1:更新(dt)
    end
    if self.动画2 then
...
function 飞升重选窗口:置模型(n)
    ...
    飞升重选窗口.动画1 = require('对象/基类/动作') { 外形 = n, 模型 = 'attack' }
    飞升重选窗口.动画1:置循环(true)
    飞升重选窗口.动画2 = require('对象/基类/动作') { 外形 = n + a, 模型 = 'attack' }
    飞升重选窗口.动画2:置循环(true)
    self.介绍文本:置文本(_说明[n])
    _外形 = n
end
...
function 窗口层:打开飞升重选窗口(r)
    飞升重选窗口:置可见(not 飞升重选窗口.是否可见)
    if not 飞升重选窗口.是否可见 then
        return
    end
    人族按钮:置选中(true)
    置角色数量(r)
