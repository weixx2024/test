...
do
    function 道具:初始化()
        ...
        self:置精灵(__res:getspr('ui/dj2.png'))
    ...
    function 道具:前显示(x, y)
        if self.动画 then
            self.动画:显示(x + 175, y + 160)
        end
    end

    function 道具:置动画模型(id)
        self.动画 = require('对象/基类/动作') { 外形 = id or __rol.原形, 模型 = 'stand', 方向 = 5 } --attack
        self.动画:置循环(true)
    end
end
道具:创建关闭按钮()
...
local 装备网格 = 道具:创建网格('装备网格', 11, 33, 340, 165)
do
    -- 左边
    -- 横着过来，披风，头盔、武器、衣服、挂件、鞋子、
    -- 右边
    -- 面具、项链、左手戒指，右手戒指、腰带，（再后一个是放五行珠，一般情况都是空的）
    function 装备网格:初始化()
        self:添加格子(4, 5, 38, 38) --1披风
        self:添加格子(64, 5, 38, 38) --2帽子
        self:添加格子(223, 5, 38, 38) --3面具
        self:添加格子(283, 5, 38, 38) --4项链
        self:添加格子(4, 61, 38, 38) --5武器
        self:添加格子(64, 61, 38, 38) --6衣服
        self:添加格子(223, 61, 38, 38) --7戒指
        self:添加格子(283, 61, 38, 38) --8戒指
        self:添加格子(4, 120, 38, 38) --9挂件
        self:添加格子(64, 120, 38, 38) --10鞋子
        self:添加格子(223, 120, 38, 38) --11腰带
        self:添加格子(283, 120, 38, 38) --12护身符
        self.数据 = {}
        self.记录格子 = 0
    end
...
if gge.platform ~= 'Windows' then
    local 使用按钮 = 道具:创建小按钮('使用按钮', 237, 203, '使用')
    function 使用按钮:左键弹起()
    ...
    local 丢弃按钮 = 道具:创建小按钮('丢弃按钮', 290, 203, '丢弃')
    function 丢弃按钮:左键弹起()
        if 道具网格.当前选中 and 道具网格.数据[道具网格.当前选中] then
            local m = 道具网格.数据[道具网格.当前选中]
            if m then
    ...
    local 解锁按钮 = 道具:创建小按钮('解锁按钮', 186, 192, '解锁')

    function 解锁按钮:初始化()
        self:置正常精灵(取按钮精灵2('ui/xx1.png', '解锁'))
        self:置按下精灵(取按钮精灵2('ui/xx3.png', '解锁', 1, 1))
        self:置经过精灵(取按钮精灵2('ui/xx2.png', '解锁'))
    end
    ...
    local 整理按钮 = 道具:创建小按钮('整理按钮', 186, 215, '整理')
    function 整理按钮:初始化()
        self:置正常精灵(取按钮精灵2('ui/xx1.png', '整理'))
        self:置按下精灵(取按钮精灵2('ui/xx2.png', '整理', 1, 1))
        self:置经过精灵(取按钮精灵2('ui/xx3.png', '整理'))
    end
    ...
    local 整理按钮 = 道具:创建小按钮('整理按钮', 237, 203, '整理')
    function 整理按钮:左键弹起()
        __rpc:角色_物品整理(道具网格.当前页)
    end

    local 摆摊按钮 = 道具:创建小按钮('摆摊按钮', 290, 203, '摆摊')
    function 摆摊按钮:左键弹起()
        if __rol.是否摆摊 then
            窗口层:打开摆摊盘点()
        else
    ...
    local 解锁按钮 = 道具:创建小按钮('解锁按钮', 185, 203, '解锁')
    function 解锁按钮:左键弹起()
        local r = 窗口层:输入窗口('', "请输入安全码")
        if r then
            __rpc:角色_解交易锁(r)
...
function 窗口层:打开道具(nid)
    ...
    local 现金, 存款, 师贡, 栏数, 原形 = __rpc:角色_打开物品窗口(nid) --你们进入现在才发现，这是改4种族就出现了的
    道具.银子文本:置文本(银两颜色(现金))
    道具.存款文本:置文本(银两颜色(存款))
    道具.师贡文本:置文本(银两颜色(师贡))
    置物品栏数量(栏数)
    ...
    local list = __rpc:角色_装备列表(nid)
    for k, v in pairs(list) do
        local s  = require('界面/数据/物品')(v):到12装备(装备网格.数据, k)
    end
    道具.lsnid = nid
    窗口层:刷新道具(_P,nid)
    道具.物品栏1:置选中(true)
