...
local function s(t, opts)
    ...
    local function safestr(s)
        return type(s) == 'number' and (huge and snum[tostring(s)] or numformat:format(s)) or
    ...
    if opts.fixradix and ('.1f'):format(1.2) ~= '1.2' then
        ...
        safestr = function(s)
            return type(s) == 'number' and (nohuge and snum[tostring(s)] or numformat:format(s):gsub(',', '.')) or
                origsafestr(s)
        end
    end
    ...
    local function safename(path, name) -- generates foo.bar, foo[3], or foo['b a r']
        ...
        local plain = type(n) == 'string' and n:match('^[%a_\xC2-\xFD\x80-\xBF][%w_\xC2-\xFD\x80-\xBF]*$') and not keyword[n] --'^[%l%u_][%w_]*$'
    ...
    local function val2str(t, name, indent, insref, path, plainindex, level)
        ...
        local tag = plainindex and ((type(name) == 'number') and '' or name .. space .. '=' .. space) or
        ...
        if ttype == 'table' then
            ...
            for n, key in ipairs(o) do
                ...
                if opts.valignore and opts.valignore[value] or -- skip ignored values; do nothing
...
return {
    ...
    line = function(a, opts)
        return s(a, merge({ sortkeys = true, comment = false }, opts))
    end,
    block = function(a, opts)
        return s(a, merge({ indent = '  ', sortkeys = true, comment = false }, opts))
    end
}
