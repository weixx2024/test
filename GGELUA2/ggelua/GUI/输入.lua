...
-- @Last Modified time  : 2022-12-23 03:29:36
...
function GUI输入:全部选中()
    ...
    for _, v in ipairs(self._精灵) do
        v.选中 = require('SDL.精灵')(0, 0, 0, v.宽度, v.高度):置颜色(table.unpack(self._选中颜色)):置中心(v
            :取中心())
    end
    return self
end
...
local function _计算选中区域(self)
    ...
    for i, v in ipairs(self._精灵) do
        if i >= min and i < max then
            v.选中 = require('SDL.精灵')(0, 0, 0, v.宽度, v.高度):置颜色(table.unpack(self._选中颜色)):
                置中心(v:取中心())
        else
            v.选中 = nil
        end
...
function GUI控件:创建输入(name, x, y, w, h)
    assert(not (self.控件[name]), name .. ':此输入已存在，不能重复创建.')
    local r = GUI输入(name, x, y, w, h, self)
...
function GUI控件:创建编辑(name, x, y, w, h)
    assert(not (self.控件[name]), name .. ':此编辑已存在，不能重复创建.')
    local r = GUI输入(name, x, y, w, h, self)
    table.insert(self.子控件, r)
    self.控件[name] = r
    r:置模式(r.正常模式 | r.多行模式)
