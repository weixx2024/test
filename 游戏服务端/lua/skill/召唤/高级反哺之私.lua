...
function 法术:物理攻击后(攻击方,挨打方,数据,伤害)
    if self:取机率(攻击方) > math.random(100) then
        ...
        local 魔法 = math.floor(攻击方.伤害 * 效果/100)
        if 主人 then
            主人:增加魔法(魔法,2)
        end
    end
...
function 法术:法术取描述(攻击方, 挨打方)
    return string.format("昔日老鸦护雏，今日慈鸟反哺，善待宝宝得到孝心回报，宝宝将攻击别人的伤害转化为法力反哺主人。#r#W【技能介绍】#r#G物理攻击时有%s%%几率将物理伤害%s%%转化为主人的法力",self:取机率(攻击方),self:法术取效果(攻击方))
end
