...
function 法术:召唤进入战斗(攻击方,数据,目标)
    -- if 攻击方 and 攻击方.PK and 攻击方.战场.回合数 > 1 and 数据 then
        ...
        local P = dst[1]
        if P then
            local 气血 = math.floor(P.最大气血 * 0.1)
            local 魔法 = math.floor(P.最大魔法 * 0.7)
...
function 法术:法术取描述(攻击方, 挨打方)
    return "仙风道骨，卓尔不凡，为队友回复大量气血与少量法力#r#W【技能介绍】#r#G进场时（第一回合除外）回复法力百分比最低友方单位70%法力和10%气血。（仅玩家之间PK战斗有效）"
end
